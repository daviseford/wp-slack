...
class WP_Slack_Event_Manager {
	...
	public function get_events() {
		return apply_filters( 'slack_get_events', array(
			...
			'post_published_newsedge' => array(
				'action'      => 'transition_post_status',
				'description' => __( 'When a post is tagged with Newsedge', 'slack' ),
				'default'     => false,
				'message'     => function( $new_status, $old_status, $post ) {
					$notified_post_types = apply_filters( 'slack_event_transition_post_status_post_types', array(
						'post',
					...
					/*
					* Added by Davis Ford. This should terminate early 
					* and not send a notification if a post doesn't belong 
					* to category "newsedge"
					* TODO: make the category selectable
					*/
					
					if ( ! in_category('newsedge', $post->ID) ) {
						return false;
					}
